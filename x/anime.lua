...
local FARM_MODES = { "已选择", "最近", "全部" }
local TARGET_MODES = { "最近", "最高血量", "最远" }
		...
		if list then
			for _, enemy in pairs(list) do
				if enemy and enemy.Name then
					local difficulty = tostring(enemy.Type or "未知")
...
local Tabs = {
	Main = Window:AddTab("主要", "swords"),
	Secondary = Window:AddTab("次要", "sparkles"),
	Gamemodes = Window:AddTab("游戏模式", "castle"),
	Gacha = Window:AddTab("抽卡", "dices"),
	Priority = Window:AddTab("优先级", "list-ordered"),
	Webhook = Window:AddTab("通知推送", "webhook"),
	Settings = Window:AddTab("设置", "settings"),
}

local FarmGroup = Tabs.Main:AddLeftGroupbox("挂机")
...
local RoundRobinGroup = Tabs.Main:AddLeftGroupbox("轮询挂机")
...
local StarGroup = Tabs.Secondary:AddLeftGroupbox("星蛋孵化")
...
local QuestGroup = Tabs.Secondary:AddLeftGroupbox("任务")
...
local EquipGroup = Tabs.Secondary:AddRightGroupbox("自动装备")
...
local ScheduleGroup = Tabs.Gamemodes:AddLeftGroupbox("刷新时间表")
...
local SPAWN_SCHEDULE = {
	Raid = { [0] = "随机等级", [10] = "大门", [20] = "大门", [30] = "随机等级", [40] = "大门", [50] = "大门" },
	Dungeon = { [0] = "简单", [15] = "中等", [30] = "简单", [45] = "中等" },
	Trial = { [0] = "简单", [15] = "中等", [30] = "简单", [45] = "中等" },
}
...
local GachaGroup = Tabs.Gacha:AddLeftGroupbox("扭蛋")
...
local SwordGroup = Tabs.Gacha:AddRightGroupbox("剑")
...
local TitanGroup = Tabs.Gacha:AddRightGroupbox("泰坦")
...
local PassiveGroup = Tabs.Gacha:AddRightGroupbox("被动")
...
local WebhookGroup = Tabs.Webhook:AddLeftGroupbox("通知推送")
		...
		embeds = {
			{
				...
				color = 5793266,
				fields = {
					{ name = "名称", value = name, inline = false },
					{ name = "结果", value = table.concat(resultLines, "\n"), inline = false },
				},
				footer = { text = "动漫星空模拟器" },
		...
		GachaResult:Connect(function(success, _, result)
			...
			for _, roll in ipairs(rolls) do
				if type(roll) == "table" and roll.RolledRarity == "Divine" then
					...
					local itemName = item and item.Name or "神圣"
		...
		for _, gamemode in ipairs(GAMEMODES) do
			...
			if schedule then
				...
				if label then
					...
					label:SetText(gamemode.Id .. ": " .. tostring(tier) .. " 倒计时 " .. FormatCountdown(remaining))
				end
...
local SwapGroup = Tabs.Secondary:AddRightGroupbox("装备方案切换")

local SwapContexts = {
	{ Index = "SwapDungeon", Text = "地牢时切换", Mode = "Dungeon" },
	{ Index = "SwapDefense", Text = "防御时切换", Mode = "Defense" },
	{ Index = "SwapStarSummon", Text = "星蛋召唤时切换", Mode = "StarSummon" },
	{ Index = "SwapRaid", Text = "突袭时切换", Mode = "Raid" },
	{ Index = "SwapTrial", Text = "时间挑战时切换", Mode = "Trial" },
	{ Index = "SwapNormalFarm", Text = "普通挂机时切换", Mode = "NormalFarm" },
}
...
local PriorityGroup = Tabs.Priority:AddLeftGroupbox("功能优先级")
...
PriorityGroup:AddLabel("1 = 最高优先级, 7 = 最低优先级", true)
			...
			if root then
				if doFarm then
					...
					if currentWorldId then
						local mode = Options.FarmMode.Value or "最近"
						local target
