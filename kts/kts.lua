...
local Tabs = {
	crystals = Window:AddTab("水晶", "gem"),
	players = Window:AddTab("玩家", "users"),
	boulders = Window:AddTab("巨石", "mountain"),
	teleports = Window:AddTab("传送", "crosshair"),
	farming = Window:AddTab("挂机", "pickaxe"),
	movement = Window:AddTab("移动", "zap"),
}

local SettingsTab = Window:AddTab("设置", "sliders-horizontal")
...
local function reportError(context, err)
	...
	lastReport = now
	warn(string.format("[挖山] %s: %s", context, tostring(err)))
end
...
local espConn = RunService.Heartbeat:Connect(function(deltaTime)
	if statsDirty and StatsLabel then
		statsDirty = false
		StatsLabel:SetText(string.format("追踪: %d  |  显示: %d", registryCount, espCount))
	end
...
local function updateBackpackLabel()
	...
	if capacity == math.huge then
		BackpackLabel:SetText(string.format("背包 %.1f / \u{221E} 千克", used))
		return
	end

	local free = math.max(0, capacity - used)
	BackpackLabel:SetText(string.format("背包 %.1f / %.1f 千克\n剩余 %.1f 千克", used, capacity, free))
end
...
local function aimTeleport()
	if not espActive then
		Library:Notify("请先开启水晶 ESP", 3)
		return
	end

	local inst = getAimedCrystal()
	if not inst then
		Library:Notify("未瞄准水晶", 2)
		return
	end

	if teleportTo(inst) then
		Library:Notify(string.format("传送到 -> %s", crystalName(inst)), 2)
	else
		Library:Notify("传送失败", 2)
	end
end
...
local function tpToRank(scoreFn, rank, formatter)
	...
	if not entry or entry.score <= 0 then
		Library:Notify(string.format("没有第 %d 名的水晶", rank), 3)
		return
	end

	if teleportTo(entry.inst) then
		Library:Notify(string.format("传送到 #%d %s (%s)", rank, crystalName(entry.inst), formatter(entry.inst, entry.score)), 3)
	else
		Library:Notify("传送失败", 3)
	end
end
	...
	local function install()
		...
		local function syncBoulders()
			...
			eachBoulder(function(model, kind)
				...
				setLine(entry, 2, string.format("%s  \u{2022}  %s 颗水晶", info.pickaxe, info.crystals))
		...
		local function grabRunes(radius)
			...
			eachRune(root.Position, radius or GRAB_RANGE, function(owner, part)
				...
				if firePrompt(prompt) then
					fired += 1
					Library:Notify(string.format("符文: %s", runeTitle(owner)), 2)
				end
			end)
...
do
	local function install()
		local CrystalBox = Tabs.crystals:AddLeftGroupbox("水晶 ESP", "gem")
		...
		CrystalBox:AddLabel("最小价值会隐藏并跳过低于该值的水晶。示例：500k、2m、1.5b。留空则显示全部", true)
		...
		StatsLabel = CrystalBox:AddLabel("追踪: 0  |  显示: 0")
	end
...
do
	local function install()
		local PlayerBox = Tabs.players:AddLeftGroupbox("玩家 ESP", "users")
...
do
	local function install()
		local BoulderBox = Tabs.boulders:AddLeftGroupbox("巨石 ESP", "mountain")
...
do
	local function install()
		local TopBox = Tabs.teleports:AddLeftGroupbox("顶级水晶", "trophy")
		...
		for rank = 1, 3 do
			TopBox:AddButton(string.format("价值前 %d", rank), function()
		...
		for rank = 1, 3 do
			TopBox:AddButton(string.format("运气前 %d", rank), function()
		...
		for rank = 1, 3 do
			TopBox:AddButton(string.format("重量前 %d", rank), function()
				tpToRank(crystalWeight, rank, fmtWeight)
			end)
		end

		local TeleportBox = Tabs.teleports:AddRightGroupbox("传送", "crosshair")
		...
		TeleportBox:AddLabel("瞄准传送"):AddKeyPicker("AimTeleportKey", {
			Default = "F",
			NoUI = false,
			Text = "瞄准传送",
			Mode = "Always",
		...
		TeleportBox:AddButton("传送回家", function()
			if fireRemote(GoHome, "home") then
				Library:Notify("正在传送回家", 2)
			else
				Library:Notify("远程事件不可用", 2)
			end
		end)

		TeleportBox:AddButton("全部出售", function()
			if doSell() then
				Library:Notify("正在出售所有水晶", 2)
			end
		end)
	end
...
do
	local function install()
		local PickupBox = Tabs.farming:AddLeftGroupbox("拾取", "pickaxe")
		...
		BackpackLabel = PickupBox:AddLabel("背包 0.0 / 0.0 千克", true)
	end
...
do
	local function install()
		local MoveBox = Tabs.movement:AddLeftGroupbox("移动", "zap")
		...
		function Net.hop()
			...
			task.spawn(function()
				for round = 1, RETRY_LIMIT do
					...
					if choice then
						visited[choice] = true
						note(string.format("尝试 %d  %s", round, string.sub(choice, 1, 8)))
						...
						note(string.format("没有服务器  http %d", lastCode))
					end
...
do
	local function install()
		...
		local function restart()
			task.spawn(function()
				pcall(function()
					LocalPlayer:Kick("Universe: 正在重启服务器")
				end)
		...
		local function step(deltaTime)
			...
			if phase == "scan" then
				if not scanned then
					...
					if scanIndex <= #SCAN_SPOTS then
						...
						statusText = string.format("正在扫描 %d/%d", scanIndex, #SCAN_SPOTS)
			...
			if phase == "mine" then
				...
				if not center then
					...
					statusText = string.format("等待 %s", kind)
					return
				end
				...
				if hp then
					...
					statusText = string.format("挖掘 %s  剩余 %.0f 生命值", kind, hp)
					return
				end
				...
				statusText = string.format("挖掘 %s", kind)
			...
			if phase == "loot" then
				...
				if now < lootUntil then
					statusText = string.format("拾取符文  %.1fs", lootUntil - now)
					return
				end
			...
			if phase == "reset" then
				...
				statusText = "正在重启服务器"
				Library:Notify("没有剩余巨石 - 正在退出并重新加入", 3)
				stop()
				restart()
			end
		end
		...
		local FarmBox = Tabs.boulders:AddRightGroupbox("巨石挂机", "bot")

		FarmBox:AddLabel("专精于符文挂机", true)
		FarmBox:AddDivider()
		...
		local StatusLabel = FarmBox:AddLabel("空闲", true)
		local labelClock = 0
...
do
	local function install()
		...
		local function step(deltaTime)
			...
			if not loaded then
				...
				if scanIndex <= #SCAN_SPOTS then
					...
					statusText = string.format("正在加载地形 %d/%d", scanIndex, #SCAN_SPOTS)
			...
			if loot then
				...
				if lootHp and lootHp > 0 then
					...
					statusText = string.format("正在破坏水晶 %d%%", math.floor(ratio * 100))
				else
					statusText = "正在收集水晶"
				end
			...
			statusText = string.format("正在挖掘 %dm 处地表", math.floor(target.Y))
		end
		...
		local MoneyBox = Tabs.farming:AddRightGroupbox("金币挂机", "banknote")

		MoneyBox:AddLabel("加载山脉并从山顶向下挖掘", true)
		MoneyBox:AddDivider()
		...
		local StatusLabel = MoneyBox:AddLabel("空闲", true)
		local labelClock = 0
...
do
	local function install()
		local ConfigGroupbox = SettingsTab:AddLeftGroupbox("配置", "folder-cog")
		...
		ConfigGroupbox:AddButton("创建配置", function()
			local name = Library.Options.SaveManager_ConfigName.Value
			if name:gsub(" ", "") == "" then
				Library:Notify("无效的配置名称（空）", 2)
				return
			end

			local success, err = SaveManager:Save(name)
			if not success then
				Library:Notify("创建配置失败: " .. err)
				return
			end

			Library:Notify(string.format("已创建配置 %q", name))
		...
		ConfigGroupbox:AddButton("加载配置", function()
			local name = Library.Options.SaveManager_ConfigList.Value
			local success, err = SaveManager:Load(name)
			if not success then
				Library:Notify("加载配置失败: " .. err)
				return
			end
			Library:Notify(string.format("已加载配置 %q", name))
		end)

		ConfigGroupbox:AddButton("覆盖配置", function()
			local name = Library.Options.SaveManager_ConfigList.Value
			local success, err = SaveManager:Save(name)
			if not success then
				Library:Notify("覆盖配置失败: " .. err)
				return
			end
			Library:Notify(string.format("已覆盖配置 %q", name))
		end)

		ConfigGroupbox:AddButton("删除配置", function()
			local name = Library.Options.SaveManager_ConfigList.Value
			local success, err = SaveManager:Delete(name)
			if not success then
				Library:Notify("删除配置失败: " .. err)
				return
			end

			Library:Notify(string.format("已删除配置 %q", name))
			Library.Options.SaveManager_ConfigList:SetValues(SaveManager:RefreshConfigList())
			Library.Options.SaveManager_ConfigList:SetValue(nil)
		end)

		ConfigGroupbox:AddButton("刷新列表", function()
			Library.Options.SaveManager_ConfigList:SetValues(SaveManager:RefreshConfigList())
			Library.Options.SaveManager_ConfigList:SetValue(nil)
		end)

		local AutoloadLabel = ConfigGroupbox:AddLabel("当前自动加载配置: " .. SaveManager:GetAutoloadConfig(), true)

		ConfigGroupbox:AddButton("设为自动加载", function()
			local name = Library.Options.SaveManager_ConfigList.Value
			local success, err = SaveManager:SaveAutoloadConfig(name)
			if not success then
				Library:Notify("设置自动加载配置失败: " .. err)
				return
			end

			Library:Notify(string.format("已将 %q 设为自动加载", name))
			AutoloadLabel:SetText("当前自动加载配置: " .. name)
		end)

		ConfigGroupbox:AddButton("重置自动加载", function()
			local success, err = SaveManager:DeleteAutoLoadConfig()
			if not success then
				Library:Notify("重置自动加载配置失败: " .. err)
				return
			end

			Library:Notify("已取消自动加载")
			AutoloadLabel:SetText("当前自动加载配置: 无")
		end)

		local MenuGroup = SettingsTab:AddRightGroupbox("菜单", "wrench")
